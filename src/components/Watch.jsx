...
const Watch = () => {
  ...
  return (
    <div
      ...
      className={`${
        open ? "w-[84.5%]" : "w-[95%]"
      }  p-3 pl-0 py-[64px]  bg-white dark:bg-black dark:text-white flex justify-center`}
    >
      <div className={`${open ? "w-[65%]" : "w-[66%]"}`}>
      <div className="min-h-120 w-full bg-gray-200 dark:bg-white/20 rounded-xl">
          <iframe
          className="rounded-xl w-full aspect-video min-h-120"
        ...
        <div className="mt-2 ">
          ...
          <div className="w-full flex items-center justify-between mt-2">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-md font-semibold">Developer Ayush</h4>
                <h6 className="text-xs text-gray-800 dark:text-gray-400">29K subscribers</h6>
              </div>
            ...
            <div className="flex items-center gap-2 ml-6">
              ...
              <div className="p-2 px-4 font-semibold text-sm flex items-center gap-4 rounded-full dark:bg-white/20 dark:text-white bg-gray-100 text-black">
                <h5 onClick={handleshare} className="leading-none flex items-center gap-2 cursor-pointer">
        ...
        <div className="w-full px-0 p-2 mt-4">
          {videoComments &&
            videoComments.map((comment, idx) => (
              <div className="flex  gap-4 mb-3">
                <div className="w-10 h-10 bg-gray-100  dark:bg-white/20 dark:text-white shrink-0 overflow-hidden rounded-full">
                  <img className="w-full h-full" src={comment?.snippet?.topLevelComment?.snippet?.authorProfileImageUrl} alt="" />
                </div>
                <div>
                  <h4 className="text-sm font-semibold">{comment?.snippet?.topLevelComment?.snippet?.authorDisplayName}</h4>
                  <h6 className="text-sm text-gray-800 dark:text-gray-200">{comment?.snippet?.topLevelComment?.snippet?.textDisplay}</h6>
      ...
      <div className={`${open ? "w-[35%]" : "w-[30%]"} h-full p-6  pt-0`}>
        {suggestedVideo &&
          suggestedVideo.map((video, idx) => (
            <Link to={`/watch?v=${video?.id}`} key={idx} className="mb-3 w-full flex">
              ...
              <div className="px-2">
                ...
                <h3 className="text-xs font-normal dark:text-gray-400">59M views</h3>
              </div>
            </Link>
          ))}
      </div>
